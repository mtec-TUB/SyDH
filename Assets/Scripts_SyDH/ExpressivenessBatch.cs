...
public class ExpressivenessBatch : MonoBehaviour
{
    ...
    [Header("Parameters")]
    public float baseExpressiveness = 1f;
    public float symmetry = 0f;
    public float smoothness = 0.0f;

    [Header("Expressiveness Variations")]
    public float[] expressivenessValues = { 0.75f, 1.25f, 1.5f };
    ...
    IEnumerator RunBatch()
    {
        ...
        // Process each expressiveness value
        foreach (float expressivenessValue in expressivenessValues)
        {
            // Create subfolder for this expressiveness value
            string expressivenessFolder = $"Expressiveness_{expressivenessValue:F2}";
            string currentOutputFolder = Path.Combine(outputFolder, expressivenessFolder);
            Directory.CreateDirectory(currentOutputFolder);

            Debug.Log($"🎬 Processing expressiveness value: {expressivenessValue} -> Folder: {expressivenessFolder}");

            // Process each JSON file for this expressiveness value
            foreach (string jsonPath in jsonFiles)
            {
                ...
                Debug.Log($"▶️ Running {baseName} with expressiveness: {expressivenessValue}");
                ...
                using (UnityWebRequest www = UnityWebRequestMultimedia.GetAudioClip(fullPath, AudioType.WAV))
                {
                    ...
                    // Apply parameters - use the current expressiveness value
                    animator.expressivenessFactor = expressivenessValue;
                    ...
                    Debug.Log($"✅ Finished {baseName} (expressiveness: {expressivenessValue})");
        ...
        Debug.Log("🏁 All samples complete for all expressiveness values!");
    }
